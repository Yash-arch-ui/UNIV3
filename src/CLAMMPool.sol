...
contract CLAMMPool {
        ...
        */
        ...
        while (amountRemaining > 0) {
            ...
            (uint160 sqrtPriceNext, uint256 amountIn, uint256 amountOut, uint256 feeAmount) =
                SwapMath.computeSwapStep(sqrtPriceX96, sqrtPriceTarget, liquidity, amountRemaining, zeroForOne);
    ...
    function swapExactOutput(bool zeroForOne, uint256 amountOutSpecified) external {
        ...
        while (amountRemaining > 0) {
            ...
            (uint160 sqrtPriceNext, uint256 amountIn, uint256 amountOut, uint256 feeAmount) = SwapMath.computeSwapStepExactOutput(
                sqrtPriceX96, sqrtPriceTarget, liquidity, amountRemaining, zeroForOne
            );
    ...
    function getNextTick(bool zeroForOne) internal view returns (int24) {
        (int24 nextTick, bool found) = TickBitmap.nextInitializedTick(bitmap, currentTick, !zeroForOne);
    ...
    function flash(uint256 amount0, uint256 amount1, address recipient, bytes calldata data) external {
        ...
        uint256 fee0 = FullMath.mulDiv(amount0, fee, 1e6);

        uint256 fee1 = FullMath.mulDiv(amount1, fee, 1e6);
    ...
    function getFeeGrowthInside(int24 tickLower, int24 tickUpper)
        external
        view
        returns (uint256 feeInside0, uint256 feeInside1)
    {
        uint256 feeBelow0;
        uint256 feeBelow1;
        ...
        if (currentTick >= tickLower) {
            ...
            feeBelow0 = feeGrowthGlobal0 - ticks[tickLower].feeGrowthOutside0;

            feeBelow1 = feeGrowthGlobal1 - ticks[tickLower].feeGrowthOutside1;
        }
        ...
        if (currentTick < tickUpper) {
            ...
            feeAbove0 = feeGrowthGlobal0 - ticks[tickUpper].feeGrowthOutside0;

            feeAbove1 = feeGrowthGlobal1 - ticks[tickUpper].feeGrowthOutside1;
        }

        feeInside0 = feeGrowthGlobal0 - feeBelow0 - feeAbove0;

        feeInside1 = feeGrowthGlobal1 - feeBelow1 - feeAbove1;
    }
