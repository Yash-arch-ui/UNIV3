...
contract PositionManager is ERC721 {
    ...
    constructor(address _pool) ERC721("CLAMM Position", "CLP") {
        pool = _pool;
    }
    /*  function getKey(
          address owner,
    ...
    function mint(address owner, int24 tickLower, int24 tickUpper, uint256 amount0Desired, uint256 amount1Desired)
        external
    {
        ...
        emit PositionUpdated(owner, tickLower, tickUpper, int128(liquidity));
    }

    function increaseLiquidity(address owner, uint256 tokenId, uint128 liquidity) external {
        ...
        emit PositionUpdated(owner, position.tickLower, position.tickUpper, int128(liquidity));
    }

    function decreaseLiquidity(address owner, uint256 tokenId, uint128 liquidity) external {
        require(ownerOf(tokenId) == owner, "NOT_OWNER");
        require(liquidity > 0, "ZERO_LIQUIDITY");
        Position.PositionInfo storage position = positions[tokenId];
        updateFees(position);
    ...
    function collect(address owner, uint256 tokenId) external returns (uint256 amount0, uint256 amount1) {
        require(ownerOf(tokenId) == owner, "NOT_OWNER");
    ...
    function updateFees(Position.PositionInfo storage position) internal {
        ...
        (uint256 feeInside0, uint256 feeInside1) =
            ICLAMMPool(pool).getFeeGrowthInside(position.tickLower, position.tickUpper);

        uint256 fees0 = FullMath.mulDiv(feeGrowthGlobal0 - position.feeGrowthInside0Last, position.liquidity, 1e18);

        uint256 fees1 = FullMath.mulDiv(feeGrowthGlobal1 - position.feeGrowthInside1Last, position.liquidity, 1e18);
