...
library SqrtPriceMath {
    function getAmount0Delta(uint160 sqrtA, uint160 sqrtB, uint128 liquidity) internal pure returns (uint256) {
        ...
        uint256 numerator = FullMath.mulDiv(uint256(liquidity), FixedPoint96.Q96, 1);

        return FullMath.mulDiv(numerator, uint256(sqrtB - sqrtA), uint256(sqrtB) * uint256(sqrtA));
    }

    function getAmount1Delta(uint160 sqrtA, uint160 sqrtB, uint128 liquidity) internal pure returns (uint256) {
        if (sqrtA > sqrtB) {
            (sqrtA, sqrtB) = (sqrtB, sqrtA);
        }

        return FullMath.mulDiv(uint256(liquidity), uint256(sqrtB - sqrtA), FixedPoint96.Q96);
    }
    ...
    function getNextSqrtPriceFromAmount0(uint160 sqrtPrice, uint128 liquidity, uint256 amountIn)
        ...
        returns (uint160)
    {
        uint256 product = FullMath.mulDiv(amountIn, sqrtPrice, FixedPoint96.Q96);

        return uint160(FullMath.mulDiv(liquidity, sqrtPrice, uint256(liquidity) + product));
    }
    ...
    function getNextSqrtPriceFromAmount1(uint160 sqrtPrice, uint128 liquidity, uint256 amountIn)
        ...
        returns (uint160)
    {
        return uint160(uint256(sqrtPrice) + FullMath.mulDiv(amountIn, FixedPoint96.Q96, liquidity));
    }

    function getNextSqrtPriceFromInput(uint160 sqrtPriceCurrent, uint128 liquidity, uint256 amountIn, bool zeroForOne)
        internal
        pure
        returns (uint128)
    {
        if (zeroForOne) {
            return uint128(getNextSqrtPriceFromAmount0(sqrtPriceCurrent, liquidity, amountIn));
        } else {
            return uint128(getNextSqrtPriceFromAmount1(sqrtPriceCurrent, liquidity, amountIn));
        }
    }

    function getNextSqrtPriceFromOutput(uint160 sqrtPriceCurrent, uint128 liquidity, uint256 amount, bool zeroForOne)
        internal
        pure
        returns (uint160 sqrtPriceNext)
    {
        ...
        if (zeroForOne) {
            ...
            uint256 quotient = FullMath.mulDivRoundingUp(amount, Q96, uint256(liquidity));
            ...
            require(product / uint256(sqrtPriceCurrent) == amount && numerator1 > product, "PRICE_OVERFLOW");

            uint256 denominator = numerator1 - product;

            // numerator = numerator1 × sqrtPriceCurrent (512-bit, handled by FullMath)
            sqrtPriceNext = uint160(FullMath.mulDivRoundingUp(numerator1, uint256(sqrtPriceCurrent), denominator));
        }
    }
}
