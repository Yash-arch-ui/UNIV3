...
contract Oracle {
    ...
    function writeObservation(int24 currentTick) external {
        if (observations.length == 0) {
            observations.push(Observation({timestamp: uint32(block.timestamp), tickCumulative: 0}));
            return;
        }
        ...
        observations.push(
            Observation({
                ...
                tickCumulative: last.tickCumulative + int56(int24(currentTick)) * (int56(uint56(timeElapsed)))
            })
        );
    }
